...
def cleanResume(txt):
    cleanText = re.sub(r'http\S+\s', ' ', txt)  # Handles URLs
    cleanText = re.sub(r'RT|cc', ' ', cleanText)  # Removes RT and cc
    cleanText = re.sub(r'#\S+\s', ' ', cleanText)  # Removes hashtags
    cleanText = re.sub(r'@\S+', '  ', cleanText)  # Removes mentions
    cleanText = re.sub(r'[!"#$%&\'()*+,-./:;<=>?@[\\\]^_`{|}~]', ' ', cleanText)  # Explicitly escaping all special chars
    cleanText = re.sub(r'[^\x00-\x7f]', ' ', cleanText)  # Removes non-ASCII characters
    cleanText = re.sub(r'\s+', ' ', cleanText)  # Replaces multiple spaces with a single space
    return cleanText
